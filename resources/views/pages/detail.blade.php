...
--}}
                    ...
                    <div class="col-md-6">
                        <div class="_product-detail-content">
                            <p class="_p-name" style="color:{!! $product->font_color !!}"> {!! ucfirst($product->product_title) !!} ({!! ucfirst($product->product_hindi_title) !!}) 
<!--							<span>Batch No. {!! $product->batch_no !!}</span>-->
							</p>
                            <div class="_p-price-box">
                                <div class="p-list">
                                    {{--<span class="price" style="color:{!! $product->background_color !!}">
                                ...
                                <div class="_p-add-cart">
                                    <p class="best-before">Best Size: </p>
                                    @if(count($product_price) !=0)
                                        @foreach($product_price as $key=>$row)
                                            @if($key == 0)
                                                <button class="a-button-text change_sizes" style="border: 1px solid #c48484" data-size="{!! $row->size !!}" data-price="{!! $row->price !!}" data-sellprice="{!! $row->selling_price !!}"><span>{!! $row->size !!}</span></button>
                                            @else
                                                &nbsp;<button class="a-button-text change_sizes" data-size="{!! $row->size !!}" data-price="{!! $row->price !!}" data-sellprice="{!! $row->selling_price !!}"><span>{!! $row->size !!}</span></button>
                                            @endif
                                        @endforeach
                                    @endif
                                    <div class="p-list mt-3"> <span class="price"> <i class="fa fa-inr" aria-hidden="true"></i> {!! $product->selling_price !!} </span> </div>
                                    <div class="_p-qty">
                                        <span>Add Quantity</span>
                                        <div class="value-button decrease_" id="decrease" onclick="decreaseValue()">-</div>
                                        <input type="text" id="number" aria-valuemin="1" readonly aria-valuemax="10" maxlength="2" value="1" />
                                ...
                                <div class="_p-qty-and-cart">
                                    <div class="_p-add-cart">
                                        <button class="btn-theme btn buy-btn buy-now" tabindex="0" data-id="{!! $product->id !!}" data-size="{!! $product->size !!}" data-price="{!! $product->price !!}">
                                        Buy Now
                                        </button>
                                        <button class="btn-theme btn btn-success add-cart" tabindex="0" data-id="{!! $product->id !!}" data-size="{!! $product->size !!}" data-price="{!! $product->price !!}">
                                         Add to Cart
                                        </button>
                                        <button class="btn-theme wishlist-btn add-wishlist" tabindex="0" data-id="{!! $product->id !!}" data-type="1" alt="Add to Wish List">
                    ...
                    <div class="col-md-12 col-xs-12">
                        <div class="tab">
                             @if($product->about != null)
                                <button class="tablinks active" id="seven">Sources of Extraction</button>
                            @endif
                            
                            @if($product->features != null)
                                <button class="tablinks" id="one">Features </button>
                            @endif
                            ...
                            @if($product->nutritional_facts != null)
                                <button class="tablinks" id="three">Nutritional Facts </button>
                            @endif
                            @if($product->interesting_facts != null)
                                <button class="tablinks " id="six">Interesting Facts</button>
                            @endif

                            @if($product->storage_instructions != null)
                                <button class="tablinks" id="four">Storage Instructions</button>
                            @endif
...
{{--
    ...
    <div class="position-fixed bottom-0 end-0 p-3" style="z-index: 11">
        <div id="wishlistToast" class="toast text-white bg-success" role="alert" aria-live="assertive" aria-atomic="true">
            <div class="d-flex">
  ...
  {{--  <script src="{!! URL::asset('assets/js/cart-jquery.js') !!}"></script>
    ...
    <script>
        ...
        $(document).on("click", ".change_sizes", function (){
            ...
            $(".change_sizes").css({'border': '1px solid #e3e3e3'});
            ...
            $("button.add-cart").attr("data-size", size);
            $("button.add-cart").attr("data-price", selling_price);
            $("button.buy-now").attr("data-size", size);
            $("button.buy-now").attr("data-price", selling_price);
        });
    ...
    <script>
        $(document).on("click", ".add-cart", function(){
           // console.log("hello");
            ...
            $.post("{!! url('cart/add-cart-product') !!}", {'price' : price, 'size' : size, '_token': '{!! csrf_token() !!}', 'product_id': product_id, 'qty': qty}, function(html){
        ...
        $(document).on("click", ".buy-now", function(){
            var price = $(this).data("price"), size = $(this).data("size"),  product_id = $(this).data("id"), qty = $("input#number").val();
            $.post("{!! url('buy-now-product') !!}", {'price' : price, 'size' : size, '_token': '{!! csrf_token() !!}', 'product_id': product_id, 'qty': qty}, function(html){
                var obj = $.parseJSON(html);
                if(obj.code == 200){
                    $("a#cart span").text(obj.count);
        ...
        $(document).on("click", ".removeWishlist", function (){
            var id = $(this).data('id');
            $.get("{!! url('my-profile/remove-wishlist') !!}", {'id' : id}, function (html){
                $.alert("Product Removed to Wishlist Successfully.");
